...
def resolve_shape_to_proxy(shape: list[Union[int, torch.SymInt]], bound_symbols: dict[Any, Any]): # -> list[Any]:
    """
...
@register_lowering_pattern(CallFunction(aten.scatter.value, CallFunction(aten.full, KeywordArg("shape"), KeywordArg("background_val"), dtype=KeywordArg("dtype")), KeywordArg("dim"), KeywordArg("selector"), KeywordArg("val")), extra_check=scatter_upon_const_tensor_extra_check)
def scatter_upon_const_tensor(match: Match, shape, background_val, dtype, dim, selector, val): # -> TensorBox:
    """
